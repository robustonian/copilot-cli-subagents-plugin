...
function installFakeCopilotBinary(binDir) {
  ...
  const script = [
    ...
    'const availableAgents = new Set(["subagents:subagents-review", "subagents:subagents-adversarial-review", "subagents:subagents-rescue"]);',
    ...
    '}',
    '',
    'if (payload.agent && !availableAgents.has(payload.agent)) {',
    '  process.stderr.write("No such agent: " + payload.agent + ", available: " + Array.from(availableAgents).join(", ") + "\\n");',
    '  process.exit(2);',
    '}',
    '',
    'let output;',
    ...
    '} else if (payload.agent === "subagents:subagents-review") {',
    '  output = "Review (" + (payload.model ?? "default") + "): " + payload.prompt.split(/\\r?\\n/)[0];',
    '} else if (payload.agent === "subagents:subagents-adversarial-review") {',
    '  output = "Adversarial review (" + (payload.model ?? "default") + "): " + payload.prompt.split(/\\r?\\n/)[0];',
    '} else if (payload.agent === "subagents:subagents-rescue") {',
