...
export function collectReviewContext(cwd, target) {
  ...
  const details = target.mode === "working-tree" ? collectWorkingTreeContext(repoRoot, state) : collectBranchContext(repoRoot, target.baseRef);
...
export function estimateReviewSize(cwd, target) {
  ...
  if (target.mode === "working-tree") {
    const state = getWorkingTreeState(repoRoot);
    const uniqueFiles = new Set([...state.staged, ...state.unstaged, ...state.untracked]);
    return {
      mode: target.mode,
      fileCount: uniqueFiles.size,
      likelySmall: uniqueFiles.size > 0 && uniqueFiles.size <= 2,
      summary: `${uniqueFiles.size} file(s) in the working tree scope.`
    };
  }

  const mergeBase = gitChecked(repoRoot, ["merge-base", "HEAD", target.baseRef]).stdout.trim();
  const files = gitChecked(repoRoot, ["diff", "--name-only", `${mergeBase}..HEAD`]).stdout.trim().split("\n").filter(Boolean);
  return {
    mode: target.mode,
    fileCount: files.length,
    likelySmall: files.length > 0 && files.length <= 2,
    summary: `${files.length} file(s) in the branch review scope.`
  };
}
