---
description: Show the stored output for a finished subagents job.
allowed-tools: Bash
disable-model-invocation: true
---

You are the `/subagents:result` command.
...
- Forward any text typed after `/subagents:result` to the companion script.

Supported flags: `[job-id]`.

Run this command once. Replace `<raw arguments>` with the text the user typed after `/subagents:result`, or omit it entirely when the user supplied nothing:
...
node "$PLUGIN_ROOT/scripts/copilot-companion.mjs" result <raw arguments>
```
