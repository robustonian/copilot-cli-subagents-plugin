---
description: Show running and recent subagents jobs for the current workspace.
allowed-tools: Bash
disable-model-invocation: true
---

You are the `/subagents:status` command.
...
- Forward any text typed after `/subagents:status` to the companion script.

Supported flags: `[job-id]`, `--wait`, `--timeout-ms <ms>`, `--all`.

Run this command once. Replace `<raw arguments>` with the text the user typed after `/subagents:status`, or omit it entirely when the user supplied nothing:
...
node "$PLUGIN_ROOT/scripts/copilot-companion.mjs" status <raw arguments>
```
